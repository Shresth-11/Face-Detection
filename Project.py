...
# Function to extract index from numpy array
...
if img is None:
    raise ValueError("Could not load the image '2.jpg'. Please check if the file exists.")
...
if not cap.isOpened():
    raise ValueError("Could not open the camera. Please check if it's connected.")
...
try:
    ...
    raise ValueError("Could not load the shape predictor. Please check if the file exists.")

# Get triangulation for the first face
...
while True:
    ...
    # Get landmarks for the second face
    face2 = faces2[0]
    landmarks = predictor(img2_gray, face2)
    landmarks_points2 = []
    for n in range(68):
        x = landmarks.part(n).x
        y = landmarks.part(n).y
